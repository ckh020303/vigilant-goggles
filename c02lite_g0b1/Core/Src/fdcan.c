...
//{
//	if(HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &TxHeader, pdata) != HAL_OK)					//发�?�数据到TX FIFO
//	{
//		Error_Handler();
//	}
//	printf("send!\n");
...
//
//
////		Notification_flag = 1;																//设置标志�????????????
////
////		if (Notification_flag == 1)													//当标志位�????????????1时，从FIFO0缓冲器接收数�????????????
////		{
////		  HAL_FDCAN_GetRxMessage(&hfdcan, FDCAN_RX_FIFO0, &RxHeader, &(RxHeader.data));
////		  printf("接收数据\n");
////		  HAL_Delay(100);
////		  Notification_flag = 0;													//复位标志�????????????
